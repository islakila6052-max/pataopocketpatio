...
/**
 * Mobile-first admin dashboard — bookings, messages, subscribers.
 */
      ...
      <div className="px-4 sm:px-6 pt-4 pb-2 grid grid-cols-3 gap-2 sm:gap-3">
        {TABS.map((tab) => (
          <button
            ...
            className={`flex flex-col items-center gap-1 py-3 px-2 rounded-2xl border transition-all active:scale-95 cursor-pointer text-center ${
              activeTab === tab.key
          ...
          >
            ...
            <span className={`text-xs font-medium ${activeTab === tab.key ? 'text-gray-900' : 'text-gray-500'}`}>
              {tab.label}
            </span>
            <span className={`text-lg font-bold ${activeTab === tab.key ? 'text-primary-700' : 'text-gray-400'}`}>
              {counts[tab.key]}
            </span>
          </button>
        ))}
        ...
        {currentCount > 3 && (
          <div className="relative mb-4 mt-2">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 bg-white text-sm text-gray-900 placeholder-gray-400 outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100 transition"
            />
          </div>
        )}
                    ...
                    return (
                      <div key={b.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
                        ...
                        <div className="flex flex-wrap gap-2 mb-3">
                          {b.preferred_date && (
                            <span className="inline-flex items-center gap-1 text-xs text-gray-600 bg-gray-50 px-2.5 py-1 rounded-lg">
                              <CalendarDays size={12} />{b.preferred_date}
                            </span>
                          )}
                          <span className="inline-flex items-center gap-1 text-xs text-gray-600 bg-gray-50 px-2.5 py-1 rounded-lg">
                            <Users size={12} />{b.guests || 1} guest{b.guests !== 1 ? 's' : ''}
                          </span>
                        </div>

                        {b.notes && (
                          <p className="text-xs text-gray-600 bg-gray-50 rounded-xl p-3 mb-3 italic leading-relaxed">
                            "{b.notes}"
                          </p>
                        )}
                        ...
                        <div className="flex gap-2 pt-1 border-t border-gray-50">
                          {b.status === 'pending' && (
                            <>
                              <button onClick={() => updateBookingStatus(b.id, 'confirmed')} className="flex-1 py-2 text-xs font-semibold bg-emerald-50 text-emerald-700 rounded-xl hover:bg-emerald-100 active:scale-95 transition-all cursor-pointer border-none">
                                ✓ Confirm
                              </button>
                              <button onClick={() => updateBookingStatus(b.id, 'cancelled')} className="flex-1 py-2 text-xs font-semibold bg-red-50 text-red-600 rounded-xl hover:bg-red-100 active:scale-95 transition-all cursor-pointer border-none">
                                ✕ Cancel
                              </button>
                            </>
                          )}
                          {b.status === 'confirmed' && (
                            <button onClick={() => updateBookingStatus(b.id, 'completed')} className="w-full py-2 text-xs font-semibold bg-blue-50 text-blue-700 rounded-xl hover:bg-blue-100 active:scale-95 transition-all cursor-pointer border-none">
                              ✓ Mark Completed
                            </button>
                          )}
                          {(b.status === 'completed' || b.status === 'cancelled') && (
                            <span className="w-full py-2 text-xs text-center text-gray-400 italic">
                              {b.status === 'completed' ? 'Visit completed' : 'Booking cancelled'}
                            </span>
                          )}
                        </div>
                      </div>
                      ...
                      {filteredSubscribers.map((s) => (
                        <div key={s.id} className="bg-white rounded-2xl p-3.5 shadow-sm border border-gray-100 flex items-center justify-between">
                          <div className="flex items-center gap-3 min-w-0">
                            <div className="w-9 h-9 rounded-xl bg-primary-50 flex items-center justify-center text-primary-700 font-semibold text-xs flex-shrink-0">
