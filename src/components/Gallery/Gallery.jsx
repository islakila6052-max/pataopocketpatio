...
export default function Gallery() {
  ...
  const [lightbox, setLightbox] = useState(null); // { plant, index }
  ...
  const nextSlide = () => {
    if (!lightbox) return;
    setLightbox(prev => ({ ...prev, index: (prev.index + 1) % prev.plant.images.length }));
  };
  const prevSlide = () => {
    if (!lightbox) return;
    setLightbox(prev => ({ ...prev, index: (prev.index - 1 + prev.plant.images.length) % prev.plant.images.length }));
  };
    ...
    <section id="gallery" className="container section-padding">
      ...
      <div className="flex flex-col items-center gap-3 mb-8">
        ...
        <p className="text-[11px] text-neutral-400 flex items-center gap-1.5">
          ...
          <span>{filtered.length} plant{filtered.length !== 1 ? 's' : ''}</span>
        </p>
      </div>
            ...
            <motion.div key={plant.id} layout variants={cardVariants} initial="hidden" animate="visible" exit="exit" custom={i}
              ...
              <div className="relative aspect-[4/5] sm:aspect-[3/4] overflow-hidden">
                ...
                <div className="absolute bottom-1.5 right-1.5">
                  <span className="px-1.5 py-0.5 rounded-md bg-black/30 backdrop-blur-sm text-white/80 text-[10px] font-medium">
            ...
            <>
              ...
              <div className="flex flex-col items-center gap-2 z-20" onClick={e => e.stopPropagation()}>
                ...
                <p className="text-white/70 text-xs sm:text-sm font-medium">
                  {lightbox.plant.name} · {lightbox.index + 1} of {lightbox.plant.images.length}
                </p>
              </div>
            </>
          )}
