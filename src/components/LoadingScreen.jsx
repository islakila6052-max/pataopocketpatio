...
export default function LoadingScreen({ onFinish }) {
  ...
  return (
    <div
      className={`fixed inset-0 z-[99999] flex flex-col items-center justify-center bg-primary-900 transition-opacity duration-700 ${
        phase === 'exit' ? 'opacity-0 pointer-events-none' : 'opacity-100'
      }`}
    >
      <div className="relative mb-6">
        <div className="w-20 h-20 rounded-2xl bg-white/10 flex items-center justify-center backdrop-blur-sm">
          <Leaf size={40} strokeWidth={1.5} className="text-green-300" />
        </div>
      ...
      <div className="mt-8 w-48 h-1 rounded-full bg-white/10 overflow-hidden">
        <div className="h-full w-full bg-green-400 rounded-full animate-[loadingBar_1.5s_ease-in-out]" />
      </div>
    </div>
  );
}
